...
sub decode_base36 {
  ...
  my $b36 = uc $_[0];
  #$b36 =~ y/A-Z0-9/0-9A-Z/;
  my $n = Math::Base36::decode_base36($b36);
  my $bin = Math::BigInt->new($n)->as_bytes();
  return $bin
}

sub encode_base36m { # A-Z0-9
  return &encode_basen($_[0],36);
}
sub decode_base36m {
  return &decode_basen($_[0],36);
}
# ----------------------------------
sub encode_base39 {
  return &encode_basen($_[0],39);
}
sub decode_base39 {
  return &decode_basen($_[0],39);
}
# ----------------------------------
...
sub alphab {
  ...
  my $alphab;
  if ($radix == 3) {
    $alphab = 'IKM'; # partikis, partika, partikum
  } elsif ($radix == 4) {
    $alphab = 'TCAG'; # human DNA
  } elsif ($radix < 12) {
    $alphab = '0123456789-';
  } elsif ($radix == 12) {
    $alphab = '0123456789XE';
    ...
    $alphab = '-0123456789ABCDEFGHiJKLMNoPQRSTUVWXYZ_'; 
  } elsif ($radix <= 39) {

...
sub mfs_append {
  ...
  my $status = &ipms_get_api('files/rm',$mpath);
      # printf "rm: %s.\n",YAML::Syck::Dump($status);
  my $mh = &ipms_post_api('files/write',$mpath,$buf,'&create=true&truncate=false');
     $mh = &ipms_get_api('files/stat',$mpath,'&hash=true');
  return $mh;
}
sub mfs_copy {
   my $src = shift;
...
# -----------------------------------------------------
sub ipfs_post_api { # add or raw-leaves of data !
  my $cmd = shift;
  my $data = shift;
  my $filename = 'blob.data';
  my $filepath = '/tmp/blob.data';
  if ($cmd eq 'add') { # /!\ ONLY ADD SUPPORTED 
     my $api_url;
